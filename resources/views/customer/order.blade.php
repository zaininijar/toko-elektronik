<x-customer-layout title="Orders">
                            ...
                            <div class="justify-between rounded-lg bg-white p-3 border sm:flex sm:justify-start">
                                <div class="w-3/12">
                                    <div class="w-12 h-12 overflow-hidden relative rounded-full">
                                        <img src="{{ Str::startsWith($order_product->product->picture_path, 'https://') ? $order_product->product->picture_path : asset('storage/' . $order_product->product->picture_path) }}"
                                            alt="{{ $order_product->product->name }}" loading="lazy"
                                ...
                                <div class="w-7/12 mt-5 sm:mt-0">
                                    ...
                                    <h3 class="text-xs font-bold text-gray-900">
                                        Rp{{ number_format($order_product->product->price * $order_product->qty, 2, ',', '.') }}
                                    </h3>
                                    <p class="mt-1 text-xs text-gray-700">
                                        {{ $order_product->product->spesification }}</p>
                                </div>
                                <div class="w-2/12 h-12 border-gray-100">
                                    <div
                                        ...
                                        <span>{{ $order_product->qty }}</span> <span>x</span>
                                    </div>
                                </div>
                            </div>
                            @endforeach
                        ...
                        <div class="ml-4 flex w-full justify-between items-start">
                            <div class="mt-0 border rounded-lg p-3 w-full">
                                <h2 class="font-bold text-xl underline">
                                    {{ $order->id }}#{{ preg_replace('/\D/', '', $order->created_at) }}</p>
                                </h2>
                                <div class="flex flex-col items-start w-full gap-1 mt-3">
                                    ...
                                    <div class="text-sm font-bold text-gray-900 gap-2 w-full flex items-start">
                                        ...
                                        <div>
                                            :
                                            {{ $order->payment->account_number . ' AN.' . $order->payment->account_name }}
                                        </div>
                                    </div>
                                    <div class="text-sm font-bold text-gray-900 gap-2 w-full flex items-start">
                                        ...
                                        <div>
                                            :
                                            {{ $store_rek[$order->payment->payment_type]['bank_name'] . ' | ' . $store_rek[$order->payment->payment_type]['account_number'] . ' AN.' . $store_rek[$order->payment->payment_type]['account_name'] }}
                                        </div>
                                    </div>
                                    <div class="text-sm font-bold text-gray-900 gap-2 w-full flex items-start">
                                        <span class="w-1/4">Total Tagihan</span>
                                        <div>
                                            : Rp{{ number_format($order->payment->total_amount, 2, ',', '.')  }}
                                        </div>
                                    </div>
                                    <div class="text-sm font-bold text-gray-900 gap-2 w-full flex items-start">
                                        <span class="w-1/4">
